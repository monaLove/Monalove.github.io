...
module.exports = function initChartbeat(config) {
  ...
  function _getChartbeatSections() {
    ...
    var subsection = (!!window.wp_subsection) ? wp_subsection : (TWP.Data&&TWP.Data.Tracking) ? TWP.Data.Tracking.props.subsection : "";
  ...
  function _getChartbeatTitle() {
    var chartbeatTitle = "";
    if (window.wp_meta_data && wp_meta_data.isHomepage) {
      chartbeatTitle = "Homepage";
    }
    return chartbeatTitle;
  }
...
},{"../util":35}],6:[function(require,module,exports){
arguments[4][4][0].apply(exports,arguments)
},{"./init":7,"dup":4}],7:[function(require,module,exports){
var _utils = require('../util');
...
},{"../util":35}],8:[function(require,module,exports){
module.exports = {
...
},{"./implementation":8,"./init":10}],10:[function(require,module,exports){
...
var ElementTracker = require('./element-tracker');
var initReport     = require('./init-report');
var trackScrolling = require('./track-scrolling');
var scrollDepth    = require('./scroll-depth');
var _ads            = require('./ads');
var _utils         = require('./util');
var newsletterHist = require('./newsletter-history');

var _omniture = require('./omniture');
var _comscore = require('./comscore');
var _chartbeat = require('./chartbeat');
var _effectiveMeasure = require('./effective-measure');
var _signalTracker = require('./signal-tracker');
var _loxodo = require('./lox');
...
(function($){
	...
	TWP.Analytics.init = TWP.Analytics.init || function(config) {
		...
		if (!! TWP.Analytics.config.service.moat ) {
			_signalTracker.init();
		}
...
},{"./ads":3,"./chartbeat":4,"./comscore":6,"./effective-measure":9,"./element-tracker":11,"./init-report":13,"./lox":15,"./newsletter-history":17,"./omniture":19,"./scroll-depth":24,"./signal-tracker":25,"./track-scrolling":28,"./util":35}],13:[function(require,module,exports){
var _utils = require('./util');
...
},{"../util":35,"./logger":16}],15:[function(require,module,exports){
var implementation = require('./implementation');
...
function init() {
  var newsletter = utils.getQueryParam(NEWSLETTER_PARAM);
  var hist;
  if (newsletter) {
    hist = getHist();
...
var newsletterHistory = require('../newsletter-history');
...
snowflake.parseState(location.search);
var viewCode = snowflake.viewCode;
var referralCode = snowflake.referralCode;

var blogname = (window.wp_blogname && wp_blogname) || (window.TWP && TWP.Data && TWP.Data.Tracking && TWP.Data.Tracking.props && TWP.Data.Tracking.props.blogname);
var section = (window.wp_section && wp_section) || (window.TWP && TWP.Data && TWP.Data.Tracking && TWP.Data.Tracking.props && TWP.Data.Tracking.props.section);
if ((blogname === 'post-nation') || (blogname === 'energy-environment') || (section === 'opinions')) {
  snowflake.syncState();
} else {
  snowflake.clearState();
}

var twpOmnitureInitialized = false;

// var webGLSupported = _utils.webGLSupported;
// try {
  ...
  */
  ...
  function s_doPlugins(s) {
    ...
    */
    ...
    var campaign_id = s.getQueryParam('campaign_id');
    var campaign_type = s.getQueryParam('campaign_type');
  ...
  */
  ...
  s.initForTwp = function() {
    ...
    */
    ...
    s.eVar57 = newsletterHistory.getHistLength() || 'none';
  ...
  s.sendPageViewToComscore = function() {
    ...
    try {
      ...
      if ( !! window.location.hostname.match(/\.washingtonpost\.com/)) {
        ...
        pvcUrl = "https://js.washingtonpost.com/wp-stat/comscore/pageview_candidate.txt?comscorekw=" + encodeURIComponent(location.pathname);
  ...
  s.isNotIframe = function(){
    return (self === top && !/embedContext=/.test(location.search))
        || (self !== top && !!self.document.referrer.match(/https?:\/\/([^\.]+\.)?(stumbleupon|jungroup|hyprmx|flickr|snip|wapo)\.(com|ly|st)\//) )
        || (window.location && /outputType=tracking/.test(location.search))
        || (window.location && /resType=tracking/.test(location.search));
  }
  ...
  function waitForPaywallTracking(timer) {
    _utils.oneWithTimeout(window.document, 'onTwpMeterComplete.omniture', initOmniture, timer || 3000);
  }
...
},{"../effective-measure":9,"../lox":15,"../newsletter-history":17,"../snowflake":26,"../util":35,"./logger":21,"./state":22}],19:[function(require,module,exports){
var init  = require('./init');
...
},{"./init":20,"./state":22}],20:[function(require,module,exports){
  ...
  if(includeOmniture){
    ...
    if (!!window.s){
      ...
      TWP.Analytics.trackScrolling(
        window.TWP && window.TWP.Analytics && window.TWP.Analytics.config && window.TWP.Analytics.config.trackScrolling
      );
    }
  } // if(includeOmniture)
...
},{"./implementation":18}],21:[function(require,module,exports){
var _utils = require('../util');
module.exports = new _utils.Log({
  namespace: "OMNITURE",
  fmtOptions: {
...
},{"../state-store":27}],23:[function(require,module,exports){
...
},{"object-assign":1,"strict-uri-encode":2}],24:[function(require,module,exports){
var _utils = require('./util');
...
function Snowflake(config) {
  config = config || {};
  this.param = config.param || DEFAULT_PARAM;
  this.queryParams = {};
}

Snowflake.prototype.parseState = function parseState(search) {
  search = search || window.location.search;
  var queryParams = qs.parse(qs.extract(search), {decode: false});
  var viewCode = newSnowflakeID();
  var refCode = queryParams[this.param];
  this.referralCode = (refCode && /^\.[0-9a-zA-Z]{12}$/.test(refCode)) ? refCode : '';
  this.viewCode = viewCode;
  queryParams[this.param] = viewCode;
  this.queryParams = queryParams;
};

Snowflake.prototype.syncState = function syncState() {
  utils.history.replaceSearch(qs.stringify(this.queryParams, {encode: false, strict: false}));
};

Snowflake.prototype.clearState = function() {
  delete this.queryParams[this.param];
  this.syncState();
};

function newSnowflakeID(){
  var d = new Date().getTime();
...
},{"./query-string":23,"./util":35}],27:[function(require,module,exports){
var _utils = require('./util');
...
/* eslint-disable */
function oneWithTimeout(selector, event, func, ms) {
  var timeout;
  ...
  jQuery(selector).one(event, handler);
  timeout = setTimeout(function() {
    jQuery(selector).off(event, handler);
    func();
  }, ms);
}

module.exports = {
  oneWithTimeout: oneWithTimeout
};
...
module.exports = function getQueryParam(name, search) {
  search = search || window.location.search;
...
exports.replaceSearch = function replaceSearch(newSearch) {
	newSearch = newSearch || '';
  ...
  var newFilename = filename + newSearch + hash;
  if (window.history && typeof window.history.replaceState === 'function') {
  	window.history.replaceState({}, document.title, newFilename);
  }
};
...
var isExternalEmbed = require('./externalEmbed');
...
if (runtime.hasStorage) {
  ...
  persistValue = function(key, val) { return cookie.createCookie(key, val, 30); };
  readValue = function(key) { return cookie.readCookie(key); };
  deleteValue = function(key) { return cookie.deleteCookie(key); };
}

module.exports = {
  hasStorage: runtime.hasStorage,
  hasJSON: runtime.hasJSON,
  hasConsole: runtime.hasConsole,
  debugMode: runtime.debugMode,
  shouldLog: runtime.shouldLog,
  // webGLSupported: runtime.webGLSupported,
  debounce: debounce,
  ...
  oneWithTimeout: events.oneWithTimeout
};

},{"./cookie":29,"./debounce":30,"./events":31,"./externalEmbed":32,"./getQueryParam":33,"./history":34,"./loadScript":36,"./logger":37,"./mobile":38,"./runtime":39}],36:[function(require,module,exports){
